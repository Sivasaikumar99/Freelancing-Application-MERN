...
const Freelancer = () => {
  ...
  const updateUserData = async () => {
    try {
      ...
      alert('User data updated')
      setIsDataUpdateOpen(false) // close the edit panel
      fetchUserData(localStorage.getItem('userId')) // refetch data
    } catch (err) {
  ...
  const fetchApplications = async (userId) => {
    try {
      ...
      setApplicationsCount(
        response.data.filter((application) => application.freelancerId === userId)
      )
    } catch (err) {
          ...
          <div className="freelancer-details">
            {!isDataUpdateOpen ? (
              ...
              <div className="freelancer-details-update">
                ...
                <button className="btn btn-outline-success mt-3" onClick={updateUserData}>Update</button>
                <button className="btn btn-outline-danger mt-3 ms-2" onClick={() => setIsDataUpdateOpen(false)}>
                  Cancel
                </button>
              </div>
            )}
